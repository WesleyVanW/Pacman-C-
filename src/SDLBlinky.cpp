...
namespace SDLPac {
	...
	SDLBlinky::SDLBlinky(SDL_Renderer* gRenderer, SDL_Rect rect, char direction, float speed, int homeTileX, int homeTileY, int startPosX, int startPosY, Pac::Timer* t) : Pac::Ghost(direction, speed, rect.x/rect.w, rect.y/rect.h, homeTileX, homeTileY, startPosX, startPosY, t) {
		...
		SDL_Surface* loadedSurface = IMG_Load("blinky.png");
	...
	//makes blinky blink when almost done being frightened
	void SDLBlinky::blink(SDL_Texture* thisTexture) {
	...
	//sets blinky width and height
	void SDLBlinky::setDimensions(int w, int h) {
		ghostWidth = w;
		ghostHeight = h;
	}
