...
namespace SDLPac {
	...
	SDLPinky::SDLPinky(SDL_Renderer* gRenderer, SDL_Rect rect, char direction, float speed, int homeTileX, int homeTileY, int startPosX, int startPosY, Pac::Timer* t) : Pac::Ghost(direction, speed, rect.x/rect.w, rect.y/rect.h, homeTileX, homeTileY, startPosX, startPosY, t) {
		...
		SDL_Surface* loadedSurface = IMG_Load("pinky.png");
	...
	//makes pinky blink if almost done being frightened
	void SDLPinky::blink(SDL_Texture* thisTexture) {
	...
	void SDLPinky::setDimensions(int w, int h) {
		ghostWidth = w;
		ghostHeight = h;
	}
