...
namespace SDLPac {
	...
	SDLClyde::SDLClyde(SDL_Renderer* gRenderer, SDL_Rect rect, char direction, float speed, int homeTileX, int homeTileY, int startPosX, int startPosY, Pac::Timer* t) : Pac::Ghost(direction, speed, rect.x/rect.w, rect.y/rect.h, homeTileX, homeTileY, startPosX, startPosY, t) {
		...
		SDL_Surface* loadedSurface = IMG_Load("clyde.png");
	...
	//makes clyde blink when almost done being frightened
	void SDLClyde::blink(SDL_Texture* thisTexture) {
	...
	//sets clyde width and height
	void SDLClyde::setDimensions(int w, int h) {
		ghostWidth = w;
		ghostHeight = h;
	}
